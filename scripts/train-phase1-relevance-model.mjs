/**
 * Train a lightweight Phase 1 relevance classifier from a JSONL dataset.
 *
 * Usage:
 *   node scripts/train-phase1-relevance-model.mjs tmp/phase1-relevance-dataset.jsonl
 *   node scripts/train-phase1-relevance-model.mjs tmp/phase1-relevance-dataset.jsonl tmp/phase1-relevance-model.json
 */

import { mkdir, readFile, writeFile } from 'node:fs/promises';
import path from 'node:path';

const LABELS = ['fraud_relevant', 'not_fraud_relevant'];
const DEFAULT_OUTPUT_PATH = 'tmp/phase1-relevance-model.json';
...
async function loadExamples(inputPath) {
  ...
  return raw
    ...
    .map((entry) => ({
      ...entry,
      tokens: tokenizeText(entry.text)
    }))
    .filter((entry) => LABELS.includes(entry.label) && entry.tokens.length > 0);
}
...
function stratifiedSplit(examples) {
  ...
  for (const label of LABELS) {
    const subset = shuffleSeeded(examples.filter((example) => example.label === label), label.length * 19);
...
function trainModel(examples) {
  ...
  for (const label of LABELS) {
    const docCount = docCounts[label];
    const tokenTotal = tokenTotals[label];
    const logPrior = Math.log((docCount + SMOOTHING) / (totalDocs + LABELS.length * SMOOTHING));
    const denominator = tokenTotal + vocabularySize * SMOOTHING;
    const defaultLogProb = Math.log(SMOOTHING / denominator);
    const tokenLogProbs = {};

    for (const token of vocabulary) {
      const count = tokenCounts[label].get(token) ?? 0;
...
async function main() {
  ...
  if (!inputPath) {
    throw new Error('Pass the relevance dataset JSONL path.');
  }

  const examples = await loadExamples(inputPath);
  const labelCounts = LABELS.map((label) => examples.filter((example) => example.label === label).length);

  if (examples.length < 10 || Math.min(...labelCounts) < 3) {
    throw new Error('Need at least 10 examples total and at least 3 per label to train a usable relevance model.');
  }
